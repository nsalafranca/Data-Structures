// Nico Salafranca nms6dz wordPuzzle.cpp
#include "hashTable.h"
#include "timer.h"
#include <iostream>
#include <fstream>
...
int main(int argc, char* argv[]){
  ...
  int numWords = 0;
  int rows, col;
  bool grid = readInGrid(b, rows, col);
  while(getline(file, o)){
    numLines++;
  }
  //file.open();
  
  size = getNextPrime(numLines++);
  hashTable ht(size);
  timer t;
  //ht.readInput(dict);
  ifstream dictionary(dict.c_str());
  ...
  for(int r = 0; r < rows; r++){
	...
	for(int d = 0; d < 8; d++){
	  for(int w = 3; w <= 23; w++){
	    string wordy = getWordInGrid(r, c, d, w, rows, col);
	    if(wordy.length() == w){
	      if(ht.contains(wordy)){
		  if(d == 0){
		    direction = "N";
		  }
		  else if(d == 1){
